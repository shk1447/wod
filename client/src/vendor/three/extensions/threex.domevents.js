...
THREEx.DomEvents.prototype._onMove = function (eventName, mouseX, mouseY, origDomEvent) {
      ...
      var visibleObjs = this.filterVisibleTarget(boundObjs);
      var intersects = this._raycaster.intersectObjects(visibleObjs);
...
/**
      2018.06.12 추가
      이벤트 대상에서 visible속성이 true 것만 반환
*/
THREEx.DomEvents.prototype.filterVisibleTarget = function( sources ){
      var results = sources.filter( function(target){
...
/**
 2018.06.12 추가
 이벤트 대상 중 mouseEnabled속성이 true 것만 반환
 */
THREEx.DomEvents.prototype.filterMouseEnabled = function( sources ){
      var results = sources.filter( function(target){
            var mesh = target.object;
            if (mesh.userData.mouseEnabled == undefined || mesh.userData.mouseEnabled == true) {
                  return true;
            }
            return false;
      });
...
THREEx.DomEvents.prototype._onEvent = function (eventName, mouseX, mouseY, origDomEvent) {
      ...
      // 보이는 객체만 대상으로 설정
      var visibleObjs = this.filterVisibleTarget(boundObjs);
      // 컴포넌트 내부 라벨에 사용하는 line은 대상에서 제외
      var intersects = this._raycaster.intersectObjects(visibleObjs, true);
      intersects = this.filterMouseEnabled(intersects);
      ...
      //if( objectParent != "undefined" && !objectParent.visible ) return;
